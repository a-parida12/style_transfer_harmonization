...
@torch.no_grad()
def translate_using_reference_2(nets, args, x_src, x_ref, y_ref, filename):
    N, C, H, W = x_src.size()

    print('xxxxxxxxxxxxxxxxxxxxx x_src xxxxxxxxxxxxxxxxxxxxxx')
    print(x_src.size())
    print(args.src_dir)
    p2 = os.path.split(args.src_dir)
    view = p2[1]
    ...
    print(srcname)
    #io.savemat(srcname, msrc)
    np.save(srcname, x_src.cpu().numpy())
    ...
    x_concat2 = []
    for i, s_ref in enumerate(s_ref_list):
        x_fake = nets.generator(x_src[i,:,:,:].unsqueeze(0), s_ref, masks=masks)
        x_fake_with_ref = torch.cat([x_ref[i:i+1],x_src[i,:,:,:].unsqueeze(0), x_fake], dim=0)
        x_concat += [x_fake]
        x_concat2 += [x_fake_with_ref]
   
    x_concat = torch.cat(x_concat, dim=0)
    x_concat2 = torch.cat(x_concat2, dim=0)
    print('xxxxxxxxxxxxxxxxxxx  x_concat  xxxxxxxxxxxxxxxx')
    print(x_concat.size())
    print(x_concat.dtype)
    print('xxxxxxxxxxxxxxxxxxx  x_concat2  xxxxxxxxxxxxxxx')
    print(x_concat2.size())
    x_concat_np = x_concat.cpu().numpy()
    #x_concat_np = np.mean(x_concat_np, axis=1).squeeze()
    print(x_concat_np.shape)
    #mdic = {"x_concat": x_concat_np}
    #harmname = args.result_dir + '/' + ID + '_' + view + '_harm.mat'
    harmname = args.result_dir + '/' + ID + '_' + view + '_harm.npy'
...
def translate_using_reference_3(nets, args, x_src, x_ref, y_ref, filename):
    ...
    print(x_src.size())
    msrc = {"x_src": x_src.cpu().numpy()}
    mref = {"x_ref": x_ref.cpu().numpy()}
    io.savemat(args.result_dir+'/res.mat', msrc)
    # io.savemat(args.savepath+'/res_gt.mat', msrc)
    ...
    io.savemat(args.result_dir+'/res_pred.mat', mdic)
    save_image(x_concat2, 3, filename)
    del x_concat, x_concat2, x_concat_np, mdic, msrc, mref

@torch.no_grad()
def translate_styles(nets, args, x_src,y_src, x_ref, y_ref, filename):
    N, C, H, W = x_src.size()
    ...
    masks = nets.fan.get_heatmap(x_src) if args.w_hpf > 0 else None
    s_ref = nets.style_encoder(x_ref, y_ref)
    print('xxxxxxxxxxxxxxxxxxxx s_ref xxxxxxxxxxxxxxxx')
    s_ref_list = s_ref.unsqueeze(1)
    print(s_ref_list.shape)
    s_src = nets.style_encoder(x_src, y_src)
    print('yyyyyyyyyyyyyyyyyyyyy s_src yyyyyyyyyyyyyyyyyyy')
    print(s_src.shape)
    mdic = {"styles": s_src.cpu().numpy()}
    io.savemat('/ifs/loni/faculty/hkim/mengting/harmonization/adni_style.mat', mdic)
